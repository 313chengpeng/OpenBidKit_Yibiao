...
async function loadDifyOutlineKnowledgeItems(difyKnowledgeService, datasetIds, query, log) {
  if (!datasetIds.length) return [];
  if (!difyKnowledgeService?.retrieve) {
    throw new Error('Dify 知识库服务尚未初始化');
  }
  log(`正在从 ${datasetIds.length} 个 Dify 知识库检索目录参考资料。`, OUTLINE_PROGRESS.knowledgeLoadStart);
  ...
  log(references.length
    ? `Dify 已召回 ${references.length} 条目录参考资料。`
    : 'Dify 未召回相关资料，将按普通目录生成。', OUTLINE_PROGRESS.knowledgeLoadEnd);
  return references;
}

function buildOutlineDifyQuery({ overview, requirements, outline }) {
  const titles = [];
...
async function runOutlineGenerationTask({ aiService, agentService, workspaceStore, knowledgeBaseService, difyKnowledgeService, updateTask, payload }) {
  let logs = ['开始生成目录。'];
  ...
  const knowledgeItems = knowledgeSourceMode === 'dify'
    ? await loadDifyOutlineKnowledgeItems(
      difyKnowledgeService, referenceDifyDatasetIds, buildOutlineDifyQuery({ overview, requirements, outline }), log,
    )
